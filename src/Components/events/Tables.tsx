...
import { addTable } from "../../store/slice/bookingSlice"
import { styles } from "../../utils/styles"

export const Tables = (table: ITables) => {
    ...
    return (
        <section id='tickets' className={`border-red-primary border-t-[1px] rounded-b-[4px] font-nunito mb-4`}>
            <div onClick={showTicket} className="flex justify-between items-center bg-[#2D2D2D] p-3 cursor-pointer">
                <h4 id='tickets' className="font-normal text-base">{table?.name}</h4>
            ...
            <div className="hidden" ref={ticketMenu}>
                <div className="flex justify-between items-center gap-4 flex-wrap py-3 px-4 border-b-[#F8F8F81A] border-b-[1px]">
                    <div>
                        ...
                        <button onClick={() => dispatch(addTable(table))} className={`${styles.primaryBtn} py-[7px] mt-2`}>
                            Select
                        </button>
                    </div>
                    ...
                    <div>
                        ...
                        <p className="text-xs font-light">No of Guests</p>
                        <p className="text-lg">{table?.numberOfGuest}</p>
                    </div>
                </div>
            </div>
        </section>
